                ...
                <div className="flex-1 overflow-y-auto p-8">
                    <div className="max-w-6xl mx-auto space-y-8 pb-10">
                        ...
                        <div className="flex justify-between items-end">
                            <div><h2 className="text-2xl font-bold text-slate-800">Dashboard Overview</h2><p className="text-sm text-slate-500 mt-1">Welcome back, here is what's happening today.</p></div>
                            <button className="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-blue-700 shadow-sm transition flex items-center gap-2"><ArrowUpRight className="w-4 h-4" /> Generate Report</button>
                        </div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                            ...
                            <div className="lg:col-span-2 bg-white rounded-xl border border-slate-200 shadow-sm p-6">
                                <div className="flex justify-between items-center mb-6">
                                    ...
                                    <select className="bg-slate-50 border border-slate-200 text-xs font-medium rounded-lg px-2 py-1 outline-none"><option>Last 7 Days</option></select>
                                </div>
                                    ...
                                    {lowStockItems.length > 0 ? lowStockItems.map((item) => {
                                        ...
                                        return (
                                            <div key={item.id} className="p-4 flex items-center gap-3 hover:bg-slate-50 transition cursor-pointer" onClick={() => navigate('/dashboard/products')}>
                                                ...
                                                <button className="px-3 py-1 text-xs border border-slate-200 rounded hover:bg-white hover:border-blue-300 hover:text-blue-600 transition">Restock</button>
                                            </div>
                                        );
                                    }) : (
                                        <div className="p-8 text-center">
                                            <div className="w-12 h-12 bg-emerald-50 rounded-full flex items-center justify-center mx-auto mb-2 text-emerald-500"><CheckCircle className="w-6 h-6" /></div>
                                            <p className="text-xs font-bold text-slate-600">Inventory Healthy</p>
                                            <p className="text-[10px] text-slate-400">No items are low on stock.</p>
                                        </div>
                                    )}
                                </div>
                            </div>
                        ...
                        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                            <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                                ...
                                <button onClick={() => navigate('/dashboard/orders')} className="text-xs font-bold text-blue-600 hover:text-blue-700 flex items-center gap-1">View All <ChevronRight className="w-3 h-3" /></button>
                            </div>
                            <div className="divide-y divide-slate-50">
...
const StatCard = ({ title, value, icon: Icon, color, trend }) => {
    ...
    return (
        <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] hover:border-blue-300 transition-all duration-200 group">
            <div className="flex justify-between items-start mb-3">
                <div className={`p-2.5 rounded-lg ${c.bg} ${c.text} border ${c.border}`}><Icon className="w-5 h-5" /></div>
                <span className={`text-[11px] font-bold px-2 py-1 rounded-full ${c.bg} ${c.text}`}>{trend}</span>
            </div>
            <div><p className="text-slate-500 text-xs font-semibold uppercase tracking-wide">{title}</p><h3 className="text-2xl font-bold text-slate-900 mt-1">{value}</h3></div>
        </div>
    );
};
...
const SimpleLineChart = ({ data }) => {
    ...
    return (
        <div className="w-full h-full flex flex-col justify-end relative pl-0 pr-0 pt-4">
            <svg viewBox="0 0 100 100" preserveAspectRatio="none" className="w-full h-full overflow-visible">
                ...
                <polyline points={points} fill="none" stroke="#2563EB" strokeWidth="2" vectorEffect="non-scaling-stroke" strokeLinecap="round" strokeLinejoin="round" />
                {data.map((d, i) => {
                    const x = (i / (data.length - 1)) * 100;
                    const y = 100 - (d.value / maxVal) * 80;
                    return (
                        <circle key={i} cx={x} cy={y} r="1.5" fill="#fff" stroke="#2563EB" strokeWidth="1" className="hover:r-2 transition-all cursor-pointer">
